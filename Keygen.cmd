...
echo:
echo PowerShell 没有运行. 等待解决...
echo 如果限制了PowerShell程序运行，请撤消更改.
echo:
echo 检查此页面获取帮助. %mas%troubleshoot
goto dk_done
)
...
if defined updatesblock call :dk_color %Gray% "检查 Windows Update 阻止程序        [发现]"
...
echo 未找到所需的许可证文件 %SystemRoot%\System32\spp\tokens\skus\
)
echo 确保您正在使用脚本是更新版本.
echo %mas%
echo:
...
set error=1
call :dk_color %Red% "检查Windows设置状态            [%imagestate%]"
echo "%imagestate%" | find /i "RESEAL" %nul% && (
set showfix=1
call :dk_color %Blue% "如果您在审核模式下运行它，则需要在正常模式下运行它."
)
)
...
call :dk_color2 %Blue% "可能由游戏欺骗引起. 帮助: %mas%issues_due_to_gaming_spoofers"
set error=1
set showfix=1
)
...
set error=1
set showfix=1
call :dk_color %Red% "检查 SPP 权限                [找到错误]"
call :dk_color %Blue% "%_fixmsg%"
)
...
if not defined _sid (
call :dk_color %Red% "检查用户帐户 SID              [未发现]"
)
...
%psc% $ExecutionContext.SessionState.LanguageMode
echo:
echo PowerShell无法运行，请检查...
echo 是否禁用PowerShell运行，如果禁用请取消.
echo:
echo 检查此页面以获取帮助. %mas%troubleshoot
goto dk_done
)
...
%psc% $ExecutionContext.SessionState.LanguageMode
echo:
echo PowerShell 不能运行. 请检查...
echo 如果对PowerShell程序限制，请撤消更改.
echo:
echo 检查此页面获取帮助. %mas%troubleshoot
goto ced_done
)
...
timeout /t 3 %nul1%
echo:
call :dk_color2 %Blue% "如果出现错误, 你必须先重启系统再重试" %_Yellow% " %mas%change_edition_issues"
)
%line%
